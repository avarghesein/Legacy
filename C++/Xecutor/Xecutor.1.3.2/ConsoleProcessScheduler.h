...
public:
	...
	inline CTString Name(void) { return msName; };
...
bool CConsoleProcessScheduler::End(void)
{
	...
	try { if(mOutStream) delete mOutStream; } catch(...) {} 
	try { if(mDynamicCmdLineProvider) delete mDynamicCmdLineProvider; } catch(...) {} 
	return bOk;
}
...
bool CConsoleProcessScheduler::StartProcess(void)
{
	...
	if(!moProcess.Start(sExe,sCmd)) return false; 
	DWORD dwId = 0;
