...
public:
	...
	bool DeleteItem(int ItemIndex,bool DeleteItemData = false);
	bool DeleteItem(CTString Item,bool DeleteItemData = false);
	int  SelectedIndex(void);
	CTString SelectedItem(void** SelectedItemData = 0);
	void* SelectedItemData(void);
	bool Clear(bool DeleteItemData = false);
	long Count(void);
}; 
...
bool CListBox::DeleteItem(int ItemIndex,bool DeleteItemData)
{
	...
	void *pItem = 0;
	if(mItemData.DeleteFrom((long)ItemIndex,&pItem) && DeleteItemData && pItem) delete pItem;
	return true;
}

bool CListBox::DeleteItem(CTString Item,bool DeleteItemData)
{
	if(!CChildControl::ValidWindow()) return false; 
	LRESULT lRes = SendMessage(LB_FINDSTRING,(WPARAM)-1,(LPARAM)(TCHAR*)Item); 
	if(IsBadLResult(lRes)) return false;
	return DeleteItem((int)lRes,DeleteItemData);
}
...
bool CListBox::Clear(bool DeleteItemData)
{
	if(!CChildControl::ValidWindow()) return false; 
	...
	while(!mItemData.IsEmptyList())  
		if(mItemData.DeleteFromFront(&pItem) && DeleteItemData && pItem) delete pItem;    
