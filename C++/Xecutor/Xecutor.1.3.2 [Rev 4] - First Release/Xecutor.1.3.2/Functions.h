...
CBytes mEnvtSettings;

CBytes GetEnvironmentForLocalMachine(CTString RegLocation)
{
	CBytes envt,var;
	CTString name,value,result;
	CRegistry regKey;
	
	if(!regKey.Open(KEY_LOCAL_MACHINE,RegLocation)) return envt;
	
	int index, ValueCount = regKey.ValuesCount();
	
	for(index = 0;index < ValueCount; index++)
		if(regKey.Value(index,name,value)) 
		{
			result = name + "=" + value;
			result.Convert(var,true);
			envt.Append(var);  
		}
...
FAR void* GetEnvSettings(void) { return (FAR void*)(FAR BYTE*)mEnvtSettings; }
...
DWORD LaunchApplications(PVOID ptrObj)
{
	...
	mEnvtSettings.Clear(); 
	mEnvtSettings = GetEnvironmentForLocalMachine("SYSTEM\\CurrentControlSet\\Control\\Session Manager\\Environment");
...
int PreRun(ENTServiceStatus eStat)
{
	if(eStat != StartPending) 
	{
		...
		LogEvent("Start Applications Thread Started...");
		moServer.Start(); 
	}
	return 1;
}
...
int ShutDownService(ENTServiceStatus eStat)
{
	...
	CloseHandle(hThread);
	moServer.Stop();
	CWaitableTimer::CreateTimerThread(false);   
	return 1;
}
