...
typedef int (*CompareDlgte)(void*,void*);


template<class ITEM,class KEY = ITEM> 
class CExtendedLinkedList:public CGenericQueue<ITEM>
{
	 protected:
	 ...
	 public:
			...
			bool DeleteAllItemByAddress(UINT64  Address);
			ITEM *GetItemByAddress(UINT64  Address);
			void operator=(CExtendedLinkedList<ITEM,KEY> &CopyFrom);
			
			ITEM *FindItem(KEY *Key,CompareDlgte Comparator);
};
...
template<class ITEM> bool CGenericQueueNode<ITEM>::DeleteItemPtr(void)
{
	try
	{
		if(m_ItemPtr) delete (ITEM*)m_ItemPtr; return true;
	}
	catch(...) { return false; } 
}
...
CExtendedLinkedList<ITEM,KEY>::CExtendedLinkedList(bool DeleteObjectsAfterUse):CGenericQueue<ITEM>(DeleteObjectsAfterUse) 
{}

template<class ITEM,class KEY> 
CExtendedLinkedList<ITEM,KEY>::~CExtendedLinkedList()
{} 

template<class ITEM,class KEY> 
CGenericQueueNode<ITEM> *CExtendedLinkedList<ITEM,KEY>::ParentNodeByAddress(UINT64  Address)
{
	try
	{
...
template<class ITEM,class KEY> 
ITEM *CExtendedLinkedList<ITEM,KEY>::GetItemByAddress(UINT64  Address)
{
	 WaitForSingleObject(m_MuxQueue,INFINITE); 
	 if(!m_qnodeHeadQnode) 
	 {
...
template<class ITEM,class KEY> 
bool CExtendedLinkedList<ITEM,KEY>::DeleteAllItemByAddress(UINT64  Address)
{
	while(DeleteItemByAddress(Address));
	return true;
}

template<class ITEM,class KEY> 
bool CExtendedLinkedList<ITEM,KEY>::DeleteItemByAddress(UINT64  Address)
{
	 WaitForSingleObject(m_MuxQueue,INFINITE);
	 if(!m_qnodeHeadQnode) 
	 {
...
template<class ITEM,class KEY> 
void CExtendedLinkedList<ITEM,KEY>::operator=(CExtendedLinkedList<ITEM,KEY> &CopyFrom)
{
	CloseHandle(m_MuxQueue); 
...
template<class ITEM,class KEY> 
ITEM *CExtendedLinkedList<ITEM,KEY>::FindItem(KEY *Key,CompareDlgte Comparator)
{
	...
	for(i=0;i<Cnt;i++)
		if(!Comparator(Key,(*this)[i])) return (*this)[i];
