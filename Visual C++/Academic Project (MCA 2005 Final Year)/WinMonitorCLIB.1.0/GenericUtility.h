...
HANDLE CGeneralMemoryUtility::m_hPrivateHeap=(HANDLE)HeapCreate((DWORD)0,(SIZE_T)100,(SIZE_T)0); 
...
bool CGeneralMemoryUtility::AllocateNew(TCHAR **TarObj,TCHAR *SrcObj,UINT32 NumChars)
{
	try
	{
		...
		*TarObj=new TCHAR[Length];
		for(ULONG i=0;i<Length-1;i++) (*TarObj)[i]=SrcObj[i];
		(*TarObj)[i]=_T('\0');
		return true;
	}
	catch(...) { return false; }
...
{
	try
	{	if(*ArrayObject && (HeapSize(m_hPrivateHeap,0,*ArrayObject)!=-1)) 
		{
...
bool CGeneralMemoryUtility::AllocateMem(TCHAR **TarObj,TCHAR *SrcObj,UINT32 NumOfChrsToCopy,bool IsAllocate)
{
	try
	{
		...
		for(ULONG i=0;i<NumOfChrsToCopy;i++) (*TarObj)[i]=SrcObj[i];
		(*TarObj)[i]=_T('\0');
		return true;
	}
	catch(...) { return false; }
...
bool CGeneralMemoryUtility::GetToken(TCHAR *Heirarchy,TCHAR *TokenSeperator,TCHAR **tchrToken,TCHAR **tchrSubHeirarchy)
{
	 ...
	 try
	 {
			...
			{
				 _w64 int lngIndexOfTokenSeperator=abs((long)(tchrTokenSeperator-Heirarchy));
