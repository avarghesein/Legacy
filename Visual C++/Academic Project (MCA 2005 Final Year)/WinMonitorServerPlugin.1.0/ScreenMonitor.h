...
public:
	  CScreenMonitorBase(BYTE Compression=SCREEN_MONITOR_COMPRESS_NIL);
	 ~CScreenMonitorBase();
...
void CScreenMonitorBase::Init(void)
{
	 ...
	 m_bytTypeCompression=SCREEN_MONITOR_SETCOMPRESSION(SCREEN_MONITOR_COMPRESS_NIL, m_bytTypeCompression);   
	 return;
}
...
bool CScreenMonitorBase::SaveBitmapToFile(TCHAR *FileName,bool IsCompress)
{
	...
	try
	{
		...
		{	
			...
			hFile =fopen(FileName,"wb"); 
			bool boolOk=fwrite((const void*)Buff,uintWritten,1,hFile);
			fclose(hFile);
			DeleteAll((void**)&Buff);
