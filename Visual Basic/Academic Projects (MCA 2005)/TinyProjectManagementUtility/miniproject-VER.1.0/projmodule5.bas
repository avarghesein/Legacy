...
pic.CurrentX = 5
pic.CurrentY = 16
pic.Print "BLACK->BLUE line:FROM TASK->TO TASK"
...
pth.xc = pic.Width / 2#
pth.yc = pic.Height / 2#
pth.r1 = Abs(Abs(pic.Width / 2#) - 9 * findmaxlen(pth))
pth.r2 = Abs(Abs(pic.Height / 2#) - 16)
With pth
For I = 0 To pth.task_count - 1 Step 1
    ...
    If (.g_path(I).angle >= 0 And .g_path(I).angle <= 90) Or (.g_path(I).angle >= 270 And .g_path(I).angle <= 360) Then
        ...
        pic.CurrentY = yi
    Else
        pic.CurrentX = xi - Len(.g_path(I).taskname) * 8
        pic.CurrentY = yi
    End If
        ...
        While I < .task_count
            ...
            While Not Y.EOF
                ...
                If j <> -1 Then
                    ...
                    xi = .xc + .r1 * (Cos(.g_path(I).angle * rad) + Cos(.g_path(j).angle * rad)) / 2#
                    yi = .yc + .r2 * (Sin(.g_path(I).angle * rad) + Sin(.g_path(j).angle * rad)) / 2#
                    
                    pic.Line (.xc + .r1 * Cos(.g_path(I).angle * rad), .yc + .r2 * Sin(.g_path(I).angle * rad)) _
                    -(xi, yi), RGB(0, 0, 0)
                
                    pic.Line (xi, yi)-(.xc + .r1 * Cos(.g_path(j).angle * rad), .yc + .r2 * Sin(.g_path(j).angle * rad)), RGB(0, 0, 300)
                    
                    
                    stg = Trim(Trim(Str(Val(.g_path(I).taskid))) & "->" & Trim(Str(Val(.g_path(j).taskid))))
                    xi = xi - Len(stg) * 3
                    pic.CurrentX = xi
                    pic.CurrentY = yi
                    pic.FontUnderline = False
...
Else
        ...
        While I <= critical_rows
            ...
            While j < critical_paths(I).cols
                ...
                xi = .xc + .r1 * (Cos(.g_path(k).angle * rad) + Cos(.g_path(l).angle * rad)) / 2#
                yi = .yc + .r2 * (Sin(.g_path(k).angle * rad) + Sin(.g_path(l).angle * rad)) / 2#
                
                pic.Line (.xc + .r1 * Cos(.g_path(k).angle * rad), .yc + .r2 * Sin(.g_path(k).angle * rad)) _
                    -(xi, yi), RGB(0, 0, 0)
                
                pic.Line (xi, yi)-(.xc + .r1 * Cos(.g_path(l).angle * rad), .yc + .r2 * Sin(.g_path(l).angle * rad)), RGB(0, 0, 300)
                    
                    
                    stg = Trim(Trim(Str(Val(.g_path(k).taskid))) & "->" & Trim(Str(Val(.g_path(l).taskid))))
                    xi = xi - Len(stg) * 3
                    pic.CurrentX = xi
                    pic.CurrentY = yi
                    pic.FontUnderline = False
