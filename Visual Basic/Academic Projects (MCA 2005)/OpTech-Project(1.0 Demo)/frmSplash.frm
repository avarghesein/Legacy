...
Dim tmp1 As Double
   ...
   If GetSetting("MyComputer\HKEY_CURRENT_USER\System\common\tvadel", "critic", "cval", "0") <> "0" Then
        tmp1 = CDbl(GetSetting("MyComputer\HKEY_CURRENT_USER\System\common\tvadel", "critic", "cval", "0"))
        If tmp1 >= 0.25 * 24# * 60# * 60# Then
            On Error Resume Next
            MsgBox "This is the demo version, Time expired." + vbCrLf + vbCrLf + "cannot run OpTech.", vbOKOnly, "Demo version Expired!!!"
            If dn.cn.state = adStateOpen Then dn.cn.Close
            Kill App.path + "\*.exe"
            Kill App.path + "\sdbase\*.*"
            Kill App.path + "\sdbase\*"
            Kill App.path + "\sdbase\dbOptech.mdb"
            RmDir App.path + "\sdbase"
            Kill App.path + "\HELP\*.*"
            Kill App.path + "\HELP\*"
            RmDir App.path + "\HELP"
            Unload Me
